...
## 14. Amendment 2026-05-15 — staging dir relocated to %LOCALAPPDATA%

When this spec was written, the install dir was assumed operator-chosen (e.g. `C:\Tools\SerialHop\`) and therefore user-writable. The installer (`2026-05-15-installer-design.md`) defaults the install dir to `C:\Program Files\SerialHop\`, which is **not** writable by the unprivileged panel. §4.3's claim that the download "surfaces 'permission denied — install dir not writable'" turned out to describe the common case, not a corner case. This amendment supersedes the affected paragraphs:

- **§4.3 (Download destination):** Stage downloads under `%LOCALAPPDATA%\SerialHop\updates\`, **not** the install dir. Created with `paths.EnsurePanelUpdateStagingDir()` (0o750). `SERIALHOP_LOCAL_DATA_DIR` overrides for tests. Same per-user dir is reused across upgrades; the cleanup glob in §4.4 now targets the staging dir, not the install dir.
- **§4.5 (Install):** `--update-src=<stagingDir>/<asset.Name>` (full absolute path). The elevated child no longer requires src to live in the install dir.
- **§5 step 1 (Validate):** drop the "same volume as the elevated child's exe" and "same directory" checks. Keep the `SerialHop-v*.exe` filename check and the file-exists check. The same-volume constraint is now an internal invariant of step 2 below — the elevated child enforces it itself.
- **§5 step 1.5 (Stage into install dir, new):** if `filepath.Dir(updateSrc) != installDir`, copy `updateSrc → <installDir>/<base>` via stage-and-rename (write to `.partial`, fsync, rename), then use the in-install-dir copy as the rename source for the existing step 4 swap. The original `<stagingDir>/<base>` is preserved so the panel's resume-from-disk logic can re-use it on the next launch without re-downloading.
- **§5 "No cross-volume case" bullet:** no longer true. Cross-volume between staging and install is now possible (different volumes on multi-disk installs). The new copy step handles it. Cross-volume within the install dir is still impossible by construction.

Security posture: writing to `%LOCALAPPDATA%` is reachable by any user-level code, whereas writing to `Program Files` required prior elevation. The SHA-256 verification done by the panel after download is unchanged, and the elevated child still re-validates the filename. The threat model gap — an attacker with user-level write access could swap the staged file between panel verification and elevated install — was already present pre-amendment (the elevated child did not re-checksum), is bounded by the same trust anchors as §13, and is documented for a future hardening pass (verify SHA-256 inside the elevated child).
